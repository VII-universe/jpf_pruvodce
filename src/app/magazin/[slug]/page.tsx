...
function renderSection(s: ArticleSection, i: number) {
  switch (s.type) {
    ...
    case "heading2":
      return <h2 key={i} className="text-2xl lg:text-[1.75rem] font-bold mt-12 mb-4 leading-tight" style={{ fontFamily: "var(--font-heading)", color: "#F1F5F9" }}>{s.text}</h2>;
    case "heading3":
      return <h3 key={i} className="text-xl font-bold mt-8 mb-3 leading-tight" style={{ fontFamily: "var(--font-heading)", color: "#F1F5F9" }}>{s.text}</h3>;
    case "blockquote":
      return (
        <blockquote key={i} className="my-8 pl-6" style={{ borderLeft: "3px solid #C9A84C" }}>
          <p className="text-xl leading-relaxed italic" style={{ fontFamily: "var(--font-heading)", color: "#E2E8F0" }}>{s.text}</p>
        </blockquote>
      );
    case "list":
      return (
        <ul key={i} className="my-6 space-y-3">
          {s.items?.map((item, j) => (
            <li key={j} className="flex items-start gap-3 text-[1.0625rem] leading-relaxed" style={{ color: "#94A3B8" }}>
              <span className="mt-2 shrink-0 w-1.5 h-1.5 rounded-full" style={{ background: "#C9A84C" }} />{item}
            </li>
          ))}
        </ul>
      );
    case "callout":
      return (
        <div key={i} className="my-8 p-6 rounded-2xl" style={{ background: "rgba(201,168,76,0.07)", borderLeft: "3px solid #C9A84C" }}>
          <p className="text-sm leading-relaxed font-semibold" style={{ color: "#E4C76B" }}>{s.text}</p>
        </div>
      );
    default: return null;
  }
  ...
  return (
    <div style={{ background: "#07090F", minHeight: "100vh" }}>
      ...
      <div className="relative pt-16 overflow-hidden" style={{ height: "clamp(380px, 55vh, 620px)" }}>
        ...
        <div className="absolute inset-0" style={{ background: "linear-gradient(to top, #07090F 0%, rgba(7,9,15,0.75) 45%, rgba(7,9,15,0.3) 100%)" }} />
        {/* Bottom fade to page background */}
        <div className="absolute bottom-0 inset-x-0 pointer-events-none" style={{ height: 100, background: "linear-gradient(to bottom, transparent, #07090F)", zIndex: 10 }} />
        <div className="absolute inset-x-0 bottom-0 max-w-4xl mx-auto px-6 lg:px-8 pb-10 lg:pb-14">
          ...
          <p className="text-base lg:text-lg max-w-2xl mb-6 leading-relaxed" style={{ color: "rgba(148,163,184,0.55)" }}>{article.subtitle}</p>
          <div className="flex flex-wrap gap-4 text-sm" style={{ color: "rgba(148,163,184,0.35)" }}>
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8 py-14 lg:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_240px] gap-12 items-start">
          ...
          <article className="max-w-[68ch]">
            ...
            <div className="mt-12 pt-8 flex flex-wrap gap-2" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
              {article.tags.map(t => (
                <span key={t} className="px-3 py-1.5 text-xs rounded-xl"
                  style={{ background: "rgba(255,255,255,0.05)", color: "rgba(148,163,184,0.5)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  {t}
                </span>
              ))}
            </div>
          ...
          <aside className="hidden lg:block sticky top-24 space-y-4">
            <div className="p-5 rounded-2xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <h4 className="text-xs font-bold tracking-[0.15em] uppercase mb-4" style={{ color: "rgba(148,163,184,0.35)" }}>Obsah článku</h4>
              <ul className="space-y-3">
                {article.content.filter(s => s.type === "heading2").map((s, i) => (
                  <li key={i} className="flex items-start gap-2.5">
                    <span className="mt-1.5 shrink-0 w-1.5 h-1.5 rounded-full" style={{ background: "#C9A84C" }} />
                    <span className="text-xs leading-relaxed" style={{ color: "rgba(148,163,184,0.55)" }}>{s.text}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="p-5 rounded-2xl overflow-hidden relative" style={{ background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.15)" }}>
              ...
              <h4 className="text-xs font-bold tracking-[0.15em] uppercase mb-2" style={{ color: "rgba(201,168,76,0.5)" }}>Diagnostika situace</h4>
              <p className="text-xs leading-relaxed mb-4" style={{ color: "rgba(148,163,184,0.55)" }}>Zjistěte, zda je interim management správnou volbou.</p>
        ...
        {/* Related */}
        {related.length > 0 && (
          <div className="mt-16 lg:mt-20 pt-12" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
            <h2 className="text-2xl font-bold mb-8" style={{ fontFamily: "var(--font-heading)", color: "#F1F5F9" }}>
              Související články
            </h2>
              ...
              {related.map(r => (
                <Link key={r.slug} href={`/magazin/${r.slug}`}
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <span className="text-xs font-bold uppercase tracking-wide mb-2" style={{ color: "rgba(148,163,184,0.35)" }}>{r.category}</span>
                    <h3 className="font-bold text-base leading-snug mb-2 flex-1 transition-colors group-hover:text-[#E4C76B]"
                      style={{ fontFamily: "var(--font-heading)", color: "#F1F5F9" }}>
                      {r.title}
                    </h3>
                    <div className="flex items-center justify-between">
                      <span className="flex items-center gap-1 text-xs" style={{ color: "rgba(148,163,184,0.3)" }}>
